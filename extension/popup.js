  ...
  chrome.storage.sync.get(
    ...
    async (options) => {
      ...
      if (!twitch.length) {
        ...
        twitch.forEach(
          ({ game, name, startedAt, thumbnail, title, viewers, embed }) =>
            addOnlineElement(
              name,
              "twitch",
              thumbnail,
...
function addOnlineElement(
  ...
  title.addEventListener(
    "click",
    function () {
      modules[server].openStream(
        this.parentNode.parentNode.getAttribute("data-profile")
      );
    },
    false
  );
...
function createTimeDiv(startedAt) {
  ...
  startTime.innerHTML =
    chrome.i18n.getMessage("startedAt") + " " + startedAt.toLocaleTimeString();
